<?php
	class DefFlightsGrid extends Grid
	{
		function DefFlightsGrid($obj)
		{			
			$this->setName('flights');
			$this->setPrefix('flights_');
			$this->setPageParam('flights_page');
			$this->setSortParam('flights_sort');
			$this->setObj($obj);
		...
		{
			return '<a href="' . url(OBJ, 'special1', array('id'=>$row['id'])) . '" >'.tr('flights_special1_link_'.$row['special1']).'</a>';
		}
		
		function column_special2($row)
		{
			return '<a href="' . url(OBJ, 'special2', array('id'=>$row['id'])) . '" >'.tr('flights_special2_link_'.$row['special2']).'</a>';
		}
		
		function column_delete($row)
		{
			return '<a href="' . url(OBJ, 'delete', array('id'=>$row['id'])) . '" >'.tr('flights_delete_link').'</a>';
		}
		...
		function getData()
		{
			...
			$response = Bus::call('flights', 'getArray', $set);
