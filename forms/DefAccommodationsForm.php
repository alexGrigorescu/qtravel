<?
	class DefAccommodationsForm extends Form
	{
		function DefAccommodationsForm($obj)
		{	
			...
			$e->options = array(0=>tr('accommodations_label_any_currency'));
			...
			$e->options = array(0=>tr('accommodations_label_any_location'));
			if ($obj->region_id > 0)
			{
			...
			$e->options = array('0'=>tr('accommodations_label_transport_individual'), '1'=>tr('accommodations_label_transport_plane'), '2'=>tr('accommodations_label_transport_bus'));
			$this->addElement($e);


			$e = new FormElementMultiSelect('early_booking');
			$earlyListResponse = Bus::call('vacancyOfferAdmin', 'getArray', array('group'=>'early_booking','sorting'=>'id asc'));
			$earlyList = array('non_early'=>'Fara reducere');
			foreach ($earlyListResponse['data'] as $row) {
				$earlyList[$row['id']] = $row['title'];
			}
			$e->options = $earlyList;
			$this->addElement($e);
			
			
			$e = new FormElementCombobox('accommodation_period');
			$periodListResponse = Bus::call('vacancyOfferAdmin', 'getArray', array('group'=>'accommodation_period','sorting'=>'CAST(title AS UNSIGNED) asc'));
			$periodList = array();
			foreach ($periodListResponse['data'] as $row) {
				$periodList[$row['id']] = $row['title'];
			}
			$e->options = $periodList;
			$this->addElement($e);

			$e = new FormElementCombobox('accommodation_applicability');
			$applicabilityListResponse = Bus::call('vacancyOfferAdmin', 'getArray', array('group'=>'accommodation_applicability','sorting'=>'id asc'));
			$applicabilityList = array();
			foreach ($applicabilityListResponse['data'] as $row) {
				$applicabilityList[$row['id']] = $row['title'];
			}
			$e->options = $applicabilityList;
			$this->addElement($e);
			
			$e = new FormElementCombobox('lunch_type');
			$lunchListResponse = Bus::call('vacancyOfferAdmin', 'getArray', array('group'=>'lunch_type','sorting'=>'id asc'));
			$lunchList = array(0=>' --- ');
			foreach ($lunchListResponse['data'] as $row) {
				$lunchList[$row['id']] = $row['title'];
			}
			$e->options = $lunchList;
			$this->addElement($e);

			$e = new FormElementText('lunch_type_text');
			$this->addElement($e);
