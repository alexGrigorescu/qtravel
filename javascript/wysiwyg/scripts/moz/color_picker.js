...
function drawColorPicker()
    ...
    var sHTMLColor="<table id=dropColor"+this.oRenderName+" style=\"border:solid 30px #ff5500; width:143px; height:109px; z-index:1;display:none;position:absolute;border:#9b95a6 1px solid;cursor:default;background-color:#E9E8F2;padding:2px\" unselectable=on cellpadding=0 cellspacing=0 width=145px><tr><td unselectable=on>"
    ...
    for(var i=0;i<arrColors.length;i++)
        ...
        for(var j=0;j<arrColors[i].length;j++)
            sHTMLColor+="<td onclick=\""+this.oName+".color='"+arrColors[i][j]+"';"+this.oName+".onPickColor();"+this.oName+".currColor='"+arrColors[i][j]+"';"+this.oName+".hideAll()\" onmouseover=\"this.style.border='#777777 1px solid'\" onmouseout=\"this.style.border='#E9E8F2 1px solid'\" style=\"width:13px;height:13px; cursor:default;padding:0; margin:0;border:#E9E8F2 1px solid;\" unselectable=on>"+
                "<table style=\"margin:0;padding:0;width:13px;height:13px;background-color:"+arrColors[i][j]+";border:white 1px solid\" cellpadding=0 cellspacing=0 unselectable=on>"+
	...
	sHTMLColor += "<table style='margin-left:1px;width:14px;height:14px;background:#E9E8F2;' cellpadding=0 cellspacing=0 unselectable=on>"+
    ...
    document.getElementById('msg').value = sHTMLColor;
    }
