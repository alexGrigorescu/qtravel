<?php
	class flight
	{
		...
		function index()
		{
			...
			$set = array('special1'=>'1', 'page_size'=>$pageSize, 'page'=>$page, 'sorting'=>' flights.price2 asc','price_min'=>1);
			if(intval($price) > 1) $set['price'] = intval($price);
			$set['continent_id'] = $continentIdsText;
	
			$locationsResponse = Bus::call('flights', 'getArrayPriceEur', $set);
			...
			$sitemap = array(
				...
				'url_title' => 'Bilete avion ' . $countryRegion,
		...
		function location(){
			...
			$new_layout = (strlen(trim((request('new_layout'))))>0)?request('new_layout'):'flight_location';
			...
			$locationNext = Bus::call('flights', 'getArrayPriceEur', array('read_id_next'=>$location['id'],'special1'=>1, 'sorting'=>'id asc', 'page'=>0,'page_size'=>1));
			$locationBack = Bus::call('flights', 'getArrayPriceEur', array('read_id_back'=>$location['id'],'special1'=>1, 'sorting'=>'id desc', 'page'=>0,'page_size'=>1));

			$location['description'] = preg_replace($stripStyle,"",ltrim(smart_crop(strip_tags($location['description'],'<br>'), 700),'&nbsp;,<br>'));
			
			$sitemap = array(
				'site_id' => 2,
				'url' => 'http://'.WEB.'.'.DOMAIN.'/bilet-avion/'.$location['end_region_code'].'/'.$location['code'].'.html',
			...
			if($location['end_country_id'] > 0) $set['end_country_id'] = $location['end_country_id'];
			$locationsBackResponse = Bus::call('flights', 'getArrayPriceEur', $set);
			
			$set = array('special1'=>'1', 'page_size'=>3, 'page'=>0, 'sorting'=>'id asc','read_id_next'=>$location['id'], 'thumb'=>1, 'thumb_width'=>200, 'thumb_height'=>200);
			if($location['end_country_id'] > 0) $set['end_country_id'] = $location['end_country_id'];
			$locationsNextResponse = Bus::call('flights', 'getArrayPriceEur', $set);
		...
		function country()
		{
			...
			$getEurParityByCode = Bus::call('currenciesValues', 'getEurParity', date('Y-m-d'));
			$set = array('special1'=>'1', 'page_size'=>$pageSize, 'page'=>$page, 'sorting'=>' flights.price2 asc','price_min'=>1,  'end_continent_id'=>implode(',',$continentValidIds));
			if(intval($price) > 1) $set['price'] = intval($price);
			if($countryId > 0) $set['end_country_id'] = $countryId;
			$set['group_by'] = 'regions2.code';

			$locationsResponse = Bus::call('flights', 'getDistinctCountriesOffers', $set);
		...
		function region()
		{
			...
			$set = array('special1'=>'1', 'page_size'=>$pageSize, 'page'=>$page, 'sorting'=>' flights.price2 asc','price_min'=>1,  'end_continent_id'=>implode(',',$continentValidIds));
			if(intval($price) > 1) $set['price'] = intval($price);
			if($regionId > 0) $set['end_region_id'] = $regionId;
			if($countryId > 0) $set['end_country_id'] = $countryId;
