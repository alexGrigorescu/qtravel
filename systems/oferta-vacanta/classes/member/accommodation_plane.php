<?php
	class accommodation_plane
	{
		...
		function index()
		{
			...
			$set = array('special1'=>'1', 'page_size'=>$pageSize, 'page'=>$page, 'thumb'=>1, 'thumb_width'=>200, 'thumb_height'=>200, 'sorting'=>' accommodations.special2 desc, accommodation_type_stars asc, accommodations.price asc','price_min'=>1, 'transport'=>1);
		...
		function region(){
			...
			$getEurParityByCode = Bus::call('currenciesValues', 'getEurParity', date('Y-m-d'));
			$set = array('special1'=>'1', 'page_size'=>$pageSize, 'page'=>$page, 'thumb'=>1, 'thumb_width'=>200, 'thumb_height'=>200, 'sorting'=>' accommodations.special2 desc, accommodation_type_stars asc, accommodations.price asc','price_min'=>1, 'transport'=>1, 'continent_id'=>$region['continent_id']);
			if(intval($price) > 1) $set['price'] = intval($price);
		...
		function country(){
			...
			$pageSize = 54;
			$getEurParityByCode = Bus::call('currenciesValues', 'getEurParity', date('Y-m-d'));
			$set = array('special1'=>'1', 'page_size'=>$pageSize, 'page'=>$page, 'sorting'=>' accommodations.special2 desc, accommodation_type_stars asc, accommodations.price asc','price_min'=>1, 'transport'=>1, 'continent_id'=>$country['continent_id']);
			if(intval($price) > 1) $set['price'] = intval($price);
			if($countryId > 0) $set['country_id'] = $countryId;
			if($stars > 0) $set['accommodation_type_id'] = $stars;
			...
			//get last offers
			$set = array('special1'=>'1', 'page_size'=>6, 'page'=>0, 'sorting'=>' accommodations.id desc','price_min'=>1, 'transport'=>1, 'thumb'=>1, 'thumb_width'=>200, 'thumb_height'=>200, 'continent_id'=>$country['continent_id']);
			if($countryId > 0) $set['country_id'] = $countryId;
			$locationsResponse = Bus::call('accommodations', 'getArrayPriceEur', $set);
			$lastOffers = array();
			if($locationsResponse['count'] > 0){
		...
		function get_destinations(){
			...
			$ajaxCountryRegionRows = Bus::call('regions', 'getCountryRegion', array('text'=>$filterText));
			$ajaxCountryRegion = array();

			foreach($ajaxCountryRegionRows as $ajaxCountryRegionRow){
				$ajaxCountryRegion[] = array(
					'id' 	=> $ajaxCountryRegionRow['region_id'].'_'.$ajaxCountryRegionRow['country_id'],
					'label'	=> !empty($ajaxCountryRegionRow['region_id'])?$ajaxCountryRegionRow['region_name'].' ('.$ajaxCountryRegionRow['country_name'].')':$ajaxCountryRegionRow['country_name'],
					'value'	=> !empty($ajaxCountryRegionRow['region_id'])?$ajaxCountryRegionRow['region_name'].' ('.$ajaxCountryRegionRow['country_name'].')':$ajaxCountryRegionRow['country_name']
				);
			}
			echo json_encode($ajaxCountryRegion);
		}

		function get_code_destinations(){
			$filterText = strtolower(request ('term'));
			$ajaxCountryRegionRows = Bus::call('regions', 'getCountryRegion', array('text'=>$filterText,'sorting'=>'buble_offer_vacancy DESC'));
			$ajaxCountryRegion = array();

			foreach($ajaxCountryRegionRows as $ajaxCountryRegionRow){
				$ajaxCountryRegion[] = array(
					'id' 	=> $ajaxCountryRegionRow['region_code'].'_'.$ajaxCountryRegionRow['country_code'],
					'label'	=> !empty($ajaxCountryRegionRow['region_id'])?$ajaxCountryRegionRow['region_name'].' ('.$ajaxCountryRegionRow['country_name'].')':$ajaxCountryRegionRow['country_name'],
					'value'	=> !empty($ajaxCountryRegionRow['region_id'])?$ajaxCountryRegionRow['region_name'].' ('.$ajaxCountryRegionRow['country_name'].')':$ajaxCountryRegionRow['country_name']
				);
			}
