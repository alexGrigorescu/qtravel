...
class layout extends SmartyTemplate 
{
	function display($meta = array(),$layout = 'layout')
	{
		...
		if(OBJ == 'reservation'){
			$pageCode = array($this->_tpl_vars['object'],$this->_tpl_vars['method']);
		} else {
			$pageCode = array(OBJ,MET);
			if(!empty($this->_tpl_vars['location']['transport'])){
				$pageCode[] = intval($this->_tpl_vars['location']['transport']);
			}
		}

		$metatagsResponse = Bus::call('meta', 'getArray', array('status'=>1, 'page_code'=>implode('-',$pageCode)));
		$countryRegion = '';
		$offerName = !empty($this->_tpl_vars['location']['title'])?$this->_tpl_vars['location']['title']:'';
		
		$metatags = array();
		if(strlen(trim(request('region'))) > 0){
			$countryRegion = request('region');
		} elseif(strlen(trim(request('country'))) > 0){
			$countryRegion = request('country');
		}
		if(!empty($metatagsResponse['data'])){
			foreach ($metatagsResponse['data'] as $metatag){
				$metatagAux = str_replace('{countryRegion}', ucfirst($countryRegion), $metatag['content']);
				$metatagAux = str_replace('{offerName}', ucfirst($offerName), $metatagAux);
				$metatags[] = $metatagAux;
			}
		}
	
		$this->assign('metatags', $metatags);	
		$this->assign('countryRegion', $countryRegion);
		$this->assign('offerName', $offerName);
