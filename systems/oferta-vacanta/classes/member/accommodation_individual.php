<?php
	class accommodation_individual
	{
		...
		function index()
		{
			...
			$set = array('special1'=>'1', 'page_size'=>$pageSize, 'page'=>$page, 'thumb'=>1, 'thumb_width'=>200, 'thumb_height'=>200, 'sorting'=>' accommodations.special2 desc, accommodation_type_stars asc, accommodations.price asc','price_min'=>1, 'transport'=>0, 'continent_id'=>implode(',',$continentValidIds));
			...
			if($locationsResponse['count'] > 0){
				...
				foreach($locationsResponse['data'] as $locationInfo){
					$displayEarlyPercent = '0';
					if(strlen(trim($locationInfo['early_booking_ids'])) > 0){
						$earlyTypesResponse = Bus::call('vacancyOfferAdmin', 'getEarlyInfo', $locationInfo['early_booking_ids']);
						if (!empty($earlyTypesResponse)) {
							$displayEarly = current($earlyTypesResponse);
							$displayEarlyPercent = $displayEarly['percent'];
						}
					}
			...
			$sitemap = array(
				...
				'url_title' => 'Vacante individual ' . $countryRegion,
			...
			$t->assign('searchTransport', 'individual');
		...
		function region(){
			...
			$getEurParityByCode = Bus::call('currenciesValues', 'getEurParity', date('Y-m-d'));
			$set = array('special1'=>'1', 'page_size'=>$pageSize, 'page'=>$page, 'thumb'=>1, 'thumb_width'=>200, 'thumb_height'=>200, 'sorting'=>' accommodations.special2 desc, accommodation_type_stars asc, accommodations.price asc','price_min'=>1, 'transport'=>0, 'continent_id'=>$region['continent_id']);
			if(intval($price) > 1) $set['price'] = intval($price);
		...
		function country(){
			...
			$pageSize = 54;
			$getEurParityByCode = Bus::call('currenciesValues', 'getEurParity', date('Y-m-d'));
			$set = array('special1'=>'1', 'page_size'=>$pageSize, 'page'=>$page, 'sorting'=>' accommodations.special2 desc, accommodation_type_stars asc, accommodations.price asc','price_min'=>1, 'transport'=>0, 'continent_id'=>$country['continent_id']);
			if(intval($price) > 1) $set['price'] = intval($price);
			if($countryId > 0) $set['country_id'] = $countryId;
			if($stars > 0) $set['accommodation_type_id'] = $stars;
			...
			//get last offers
			$set = array('special1'=>'1', 'page_size'=>6, 'page'=>0, 'sorting'=>' accommodations.id desc','price_min'=>1, 'transport'=>0, 'thumb'=>1, 'thumb_width'=>200, 'thumb_height'=>200, 'continent_id'=>$country['continent_id']);
			if($countryId > 0) $set['country_id'] = $countryId;
			$locationsResponse = Bus::call('accommodations', 'getArrayPriceEur', $set);
			$lastOffers = array();
			if($locationsResponse['count'] > 0){
			...
			$sitemap = array(
				'site_id' => 2,
				'url' => substr(url(OBJ,'index'),0,stripos(url(OBJ,'country',array('country'=>$code)),'?')),
				'url_md5' => md5(substr(url(OBJ,'index'),0,stripos(url(OBJ,'country',array('country'=>$code)),'?'))),
				'url_title' => 'Vacante individual ' . $countryRegion,
				'last_crawled' => date('Y-m-d H:i:s') 
			);
			Bus::call('sitemap', 'insert', $sitemap);
