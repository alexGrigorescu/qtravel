...
{if $location.price|strlen gt 0}
<span class="description-text">Preturi de la {$location.price} {$location.currency_title}</span><br /><br />
{/if}
{if $location.description|strlen gt 0}
<span class="description-text">{$location.description}</span><br /><br />
{/if}
	...
	{foreach from=$prices item=new_price_format}
		...
		{foreach from=$new_price_format.prices item=new_price}
			<tr>
				...
				{if $price_single}<td class="right">{$new_price.night.price_single_format}</td>{/if}
				{if $price_double}<td class="right">{$new_price.night.price_double_format}</td>{/if}
				{if $price_3adult}<td class="right">{$new_price.night.price_3adult_format}</td>{/if}
				{if $price_triple}<td class="right">{$new_price.night.price_triple_format}</td>{/if}
				{if $price_1child}<td class="right">{$new_price.night.price_1child_format}</td>{/if}
				{if $price_2child}<td class="right">{$new_price.night.price_2child_format}</td>{/if}
				{if $price_extra1}<td class="right">{$new_price.night.price_extra1_format}</td>{/if}
				{if $price_extra2}<td class="right">{$new_price.night.price_extra2_format}</td>{/if}
				{if $price_extra3}<td class="right">{$new_price.night.price_extra3_format}</td>{/if}
			</td>
		{/foreach}
		</table>
		<br/><br/>
...
{else }
	euro (&euro;)
{/if} si sunt calculate pe o noapte
</span><br /><br />	
