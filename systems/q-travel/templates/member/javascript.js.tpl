...
$(function() {
	$('#submenu-vacancies').append('<ul><li class="submenu"><div id="container"><a href="{/literal}{url o='accommodation_plane' m='index'}{literal}"><img class="first" src="{/literal}{$img_path}{literal}/submenu_vacancies_plane.png"></a><a href="{/literal}{url o='accommodation_bus' m='index'}{literal}"><img src="{/literal}{$img_path}{literal}/submenu_vacancies_bus.png"></a><a href="{/literal}{url o='accommodation_individual' m='index'}{literal}"><img src="{/literal}{$img_path}{literal}/submenu_vacancies_single.png"></a></div></li></ul>');
	$('#submenu-tickets').append('<ul><li class="submenu"><div id="container"><a href="{/literal}{url o='flight' m='index'}{literal}"><img class="first" src="{/literal}{$img_path}{literal}/submenu_tickets_plane.png"></a><a href="{/literal}{url o='buss' m='index'}{literal}"><img src="{/literal}{$img_path}{literal}/submenu_tickets_bus.png"></a></div></li></ul>');
...
function searchHomePage(object,method){
	...
	$.ajax({
	  type: "POST",
	  url: $("#hid-ajax-url-prefix").val() + method + ".html",
	  data: "val=11",
...
$(function() {
	...
	$("#btn-search").click(function() {
		var searchUrl = $('#hid-search-url').val();
		if($("#hid-autocomplete-home-destination-id").val() == ''){
			...
			if($("#autocomplete-vacancy-type").length){
				searchUrl += $("#hid-autocomplete-vacancy-type-id").val() + '.html';
			}
			location.href = searchUrl+'?dt='+$("#hid-autocomplete-home-destination-id").val()+'&st='+
							   $("#hid-autocomplete-stars-id").val()+'&ht='+$("#hid-autocomplete-hotel-id").val();
		}
	});
	$("#btn-search-plane").click();
});
