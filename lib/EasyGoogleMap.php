...
class EasyGoogleMap {
	...
	/**
	 * @desc: 	Enable/Disable Map Search 
	 * @type: 	boolean
	 * @access: public
	 */
	var $mSearch = FALSE;
	...
	function EasyGoogleMap($mapKey = GOOGLE_KEY) {
		$this->mMapKey = $mapKey;
		$this->SetMapWidth();
		$this->SetMapHeight();
		$this->SetMapZoom();
	...
	function SetLat($lat=300) {
		$this->mLat = $lat;
	} # end function
	
	function SetBubble($bubble = "test") {
		$this->mBubble = $bubble;
	} # end function
	
		function SetLon($lon=300) {
		$this->mLon = $lon;
	} # end function
	/**
	...
	function InitJs() {
		...
		if ($is_error) { 
			...
			$ret .= ($this->mDoubleClickZoom==FALSE)?"	map.disableDoubleClickZoom(); \n":"";
			...
			# handle map search
			$ret .= ($this->mSearch==TRUE)?" map.addControl(new google.maps.LocalSearch(), new GControlPosition(G_ANCHOR_BOTTOM_RIGHT, new GSize(10,20))); \n":"";
	...
	/*		for ($i=$cnt_add-1; $i>=0; $i--) {
				...
				$ret .= "		if(point) {\n";
				$ret .= "		  points[$i] = point; \n";	*/
		$ret .= "		point = 	new GLatLng($this->mLat, $this->mLon);\n";
				$ret .= "		  map.setCenter(point, {$this->mMapZoom});\n";
			$ret .= "		  var marker = new GMarker(point, icon);\n";
						$ret .= "		  GEvent.addListener(marker, 'click', function() {\n";
//			$ret .= "			marker.openInfoWindowHtml(point.infowindowtext);\n";
				$ret .= "		  });\n";

				$ret .= "		  map.addOverlay(marker);\n";
//				$ret .= "		marker.openInfoWindowHtml(document.createTextNode({$this->mMapBubble}));\n";
				$ret .= "   marker.openInfoWindow(document.createTextNode(\"{$this->mBubble}\"));\n";
/*	
...
//					$ret .= "		  marker.openInfoWindowHtml(point.infowindowtext);\n";
	...
	/*			$ret .= "		  gmarkers[$i] = marker;\n";
				...
				$ret .= "		}\n";
				$ret .= "		else {\n";*/
		//		$ret .= "		  map.setCenter(new GLatLng($this->mLat, $this->mLon), {$this->mMapZoom});\n";
		/*		$ret .= "		}\n";
				$ret .= "	  }\n";
	...
	//		$ret .= "map.addOverlay(new GMarker(new GLatLng($this->mLat, $this->mLon),$this->));\n";
