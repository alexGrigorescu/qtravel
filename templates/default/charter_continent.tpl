...
<a href="{url o='charter' m='continent' continent=$continent.code}">{$continent.title}</a>

{foreach from=$continents item=new_continent key=continent_code}
	<h3 class="continent">{$new_continent.title}</h3>
	...
	<td style="vertical-align:top; width:33%;">
		{foreach from=$new_continent.countries item=country name=countries}
			...
			{/if}
			<div style="padding-top:1px;"><a class="country" href="{url o='charter' m='country' country=$country.code}"> &raquo; <b>{$country.title}</b> ({$country.count})</a></div>			
		{/foreach}
	</td>
	</tr>
	</table>
...
{if $continent.description > ''}
	<div style="margin:5px;">	
		<div>
			<h3 class="continent">{tr key="charter_title_continent_description"}  {$continent.title}</h3>
