...
{if $locations_count > 1}
	<div style="margin:5px;">	
		<div>
			<h3 class="continent">{tr key="flight_title_index_flights"}</h3>
			{foreach from=$locations item=new_location}
			{if $new_location.code != $location.code}
			<div class="acc">
				<div class="acctitle">
					<div style="float:left; height:16px; width:320px;"><a href="{url o='flight' m='location' region=$new_location.end_region_code location=$new_location.code}" title="{$new_location.title}">{$new_location.title|truncate:100}</a></div>
