...
<a href="{url o='vacation' m='continent' continent=$country.continent_code}">{$country.continent_title}</a> &raquo;  
<a href="{url o='vacation' m='regions' country=$country.code}">{$country.title}</a> 
...
<td style="vertical-align:top; width:33%;">
	{foreach from=$regions item=region name=regions}
		...
		{/if}
		<div style="padding-top:2px;"><a class="country" href="{url o='vacation' m='region' region=$region.code}"> &raquo; <b>{$region.title}</b> ({$region.count})</a></div>
	{/foreach}
</td>
</tr>
</table>
...
{if $locations_count > 1}
	<div style="margin:5px;">	
		<div>
			<h3 class="continent">{tr key="vacation_title_country_vacations"}  {$country.title}, {$country.continent_title}</h3>
...
{if $country.description > ''}
	<div style="margin:5px;">	
		<div>
			<h3 class="continent">{tr key="vacation_title_country_description"}  {$country.title}, {$country.continent_title}</h3>
			{$country.description}
		</div>
	</div>
{/if}
