...
<a href="{url o='buss' m='continent' continent=$continent.code}">{$continent.title}</a>
...
{if $locations_count > 1}
	<div style="margin:5px;">	
		<div>
			<h3 class="continent">{tr key="buss_title_country_busses"}  {$continent.title}</h3>
			{foreach from=$locations item=new_location}
			{if $new_location.code != $location.code}
			<div class="acc">
				<div class="acctitle">
					<div style="float:left; height:16px; width:360px;"><a href="{url o='buss' m='location' region=$new_location.end_region_code location=$new_location.code}" title="{$new_location.title}">{$new_location.title}</a></div>
...
{if $continent.description > ''}
	<div style="margin:5px;">	
		<div>
			<h3 class="continent">{tr key="buss_title_continent_description"}  {$continent.title}</h3>
