<div>
	...
	<h3 class="acch3">{tr key="accommodation_title_accommodation"} <b style="color:#ff0000;">{$location.title}</b>, {$location.region_title}, {$location.country_title}, {$location.continent_title}</h3>
	<div>
		<div style="float:left">
			<div><img src="{$img_path}star_{$location.accommodation_type_stars}.gif"  alt="{$location.accommodation_type_title}"/></div>					
			<div><span class="price">{tr key="accommodation_price"} {$location.price} {$location.currency_title}</span></div>													
		</div>
		<div style="float:right" class="reservation"><a href="{url o='reservation' m='index' type=accommodations id=$location.id}">{tr key="accommodation_reservation"} {$location.title}</a></div>
	...
	{if $prices_count > 0}
	<h3 class="acch3">{tr key="accommodation_title_prices"} <b style="color:#ff0000;">{$location.title}</b>, {$location.region_title}, {$location.country_title}, {$location.continent_title}</h3>
	...
	* Preturile sunt exprimate in 
	...
	{/if}
	<div><br/>{$location.description|escape:"br"}</div>
	{if $locations_count > 1}
		<div>
			<h3 class="acch3">{tr key="accommodation_title_other_accommodations"} {$location.region_title}, {$location.country_title}, {$location.continent_title}</h3>
			...
			<div class="acc" style="width:100%;">
				...
				<div class="accbodyreg" style="height:115px;">
					...
					<div style="text-align:right; padding:5px;">
						...
						<span class="price">{tr key="accommodation_price"} {$new_location.price} {$new_location.currency_title}</span>
					</div>													
	...
	<div id="map">
		<h3 class="acch3">{tr key='title_map'}  {$location.region_title}, {$location.country_title}, {$location.continent_title}</h3>
