<div style="">
	<div class="accnav">
		...
		<a href="{url o=$type m='continent' continent=$location.continent_code}">{$location.continent_title}</a>  &raquo; 
		<a href="{url o=$type m='country' country=$location.country_code}">{$location.country_title}</a>  &raquo; 
		<a href="{url o=$type m='region' region=$location.region_code}">{$location.region_title}</a>
	</div>
	<h3 class="acch3">{tr key="vacation_title_vacation"} <b style="color:#ff0000;">{$location.title}</b>, {$location.region_title}, {$location.country_title}, {$location.continent_title}</h3>
	<div style="padding:5px;">
		<div style="float:left">
			<div><img src="{$img_path}star_{$location.accommodation_type_stars}.gif"  alt="{$location.accommodation_type_title}"/></div>					
			<div><span class="price">{tr key="vacation_price"} {$location.price} {$location.currency_title}</span></div>													
		</div>
		<div style="float:right" class="reservation"><a href="{url o='reservation' m='index' type=$type id=$location.id}">{$tr.type_reservation} {$location.title}</a></div>
	...
	{if $prices_count > 0}
	<h3 class="acch3">{tr key="vacation_title_prices"} <b style="color:#ff0000;">{$location.title}</b>, {$location.region_title}, {$location.country_title}, {$location.continent_title}</h3>
	...
	* Preturile sunt exprimate in 
	...
	{/if}
	{if $location.description_included > ''}
		<div><br/><b>Tariful include:</b><br/>{$location.description_included|nl2br}</div>
	{/if}
	{if $location.description_not_included > ''}
		<div><br/><b>Tariful nu include:</b><br/>{$location.description_not_included|nl2br}</div>
	{/if}
	{if $location.description_early_booking > ''}
		<div><br/><b>Early booking:</b><br/>{$location.description_early_booking|nl2br}</div>
	{/if}
	{if $location.description_special_offer > ''}
		<div style="color:red;"><br/><b>Oferta speciala:</b><br/>{$location.description_special_offer|nl2br}</div>
	{/if}
	<div>{$location.description|escape:"br"}</div>
	{if $locations_count > 1}
		<div>
			<h3 class="acch3">{tr key="vacation_title_other_vacations"} {$location.region_title}, {$location.country_title}, {$location.continent_title}</h3>
			{foreach from=$locations item=new_location}
			{if $new_location.code != $location.code}
			<div class="acc" style="width:100%;">
				<div class="acctitle">
					<div style="float:left; height:16px;"><a href="{url o=$type m='location' location=$new_location.code country=$new_location.country_code}" title="{$new_location.title}">{$new_location.title}</a></div>
					<div class="clearer"><span></span></div>
				</div>
				<div class="accbodyreg" style="height:115px;">
					<div style="float:left;width:110px; text-align:center; padding-top:5px;">
						<div class="accthumb"><a href="{url o=$type m='location' location=$new_location.code country=$new_location.country_code}" title="{$new_location.title}">{$new_location.location_thumb}</a></div>
					</div>
					<div style="text-align:right; padding:5px;">
						...
						<span class="price">{$tr.type_price} {$new_location.price} {$new_location.currency_title}</span>
					</div>													
