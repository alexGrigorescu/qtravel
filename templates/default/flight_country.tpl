...
<a href="{url o='flight' m='continent' continent=$country.continent_code}">{$country.continent_title}</a> &raquo;  
<a href="{url o='flight' m='regions' country=$country.code}">{$country.title}</a> 
...
<td style="vertical-align:top; width:33%;">
	{foreach from=$regions item=region name=regions}
		...
		{/if}
		<div style="padding-top:2px;"><a class="country" href="{url o='flight' m='region' region=$region.code}"> &raquo; <b>{$region.title}</b> ({$region.count})</a></div>
	{/foreach}
</td>
</tr>
</table>
...
{if $locations_count > 1}
	<div style="margin:5px;">	
		<div>
			<h3 class="continent">{tr key="flight_title_country_flights"}  {$country.title}, {$country.continent_title}</h3>
			{foreach from=$locations item=new_location}
			{if $new_location.code != $location.code}
			<div class="acc">
				<div class="acctitle">
					<div style="float:left; height:16px; width:320px; overflow:hidden;"><a href="{url o='flight' m='location' region=$new_location.end_region_code location=$new_location.code}" title="{$new_location.title}">{$new_location.title}</a></div>
					<div class="clearer"><span></span></div>
				</div>
				<div class="accbody">
					<div style="text-align:right; padding-right:5px;"><span class="price">{tr key="flight_price"} {$new_location.price1} {$new_location.currency_title}</span></div>													
...
{if $country.description > ''}
	<div style="margin:5px;">	
		<div>
			<h3 class="continent">{tr key="flight_title_country_description"}  {$country.title}, {$country.continent_title}</h3>
			{$country.description}
		</div>
	</div>
{/if}
