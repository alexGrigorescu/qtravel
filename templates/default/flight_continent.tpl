...
<a href="{url o='flight' m='continent' continent=$continent.code}">{$continent.title}</a>
...
{foreach from=$continents item=new_continent key=continent_code}
	...
	<td style="vertical-align:top; width:33%;">
		{foreach from=$new_continent.countries item=country name=countries}
			...
			{/if}
			<div style="padding-top:1px;"><a class="country" href="{url o='flight' m='country' country=$country.code}"> &raquo; <b>{$country.title}</b> ({$country.count})</a></div>			
		{/foreach}
	</td>
	</tr>
	</table>
...
{if $locations_count > 1}
	<div style="margin:5px;">	
		<div>
			<h3 class="continent">{tr key="flight_title_country_flights"}  {$continent.title}</h3>
			{foreach from=$locations item=new_location}
			{if $new_location.code != $location.code}
			<div class="acc">
				<div class="acctitle">
					<div style="float:left; height:16px; width:360px;"><a href="{url o='flight' m='location' region=$new_location.end_region_code location=$new_location.code}" title="{$new_location.title}">{$new_location.title}</a></div>
				...
				<div class="accbody">
					...
					<div style="height:75px; padding:0px; padding-left:10px; border:0px solid #EBEBEB; ">{$new_location.description|strip_tags|truncate:400}</div>
					<div class="clearer"><span></span></div>
				</div>
			</div>
			{/if}
...
{if $continent.description > ''}
	<div style="margin:5px;">	
		<div>
			<h3 class="continent">{tr key="flight_title_continent_description"}  {$continent.title}</h3>
