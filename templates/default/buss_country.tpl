...
<a href="{url o='buss' m='continent' continent=$country.continent_code}">{$country.continent_title}</a> &raquo;  
<a href="{url o='buss' m='regions' country=$country.code}">{$country.title}</a> 
...
<td style="vertical-align:top; width:33%;">
	{foreach from=$regions item=region name=regions}
		...
		{/if}
		<div style="padding-top:2px;"><a class="country" href="{url o='buss' m='region' region=$region.code}"> &raquo; <b>{$region.title}</b> ({$region.count})</a></div>
	{/foreach}
</td>
</tr>
</table>
...
{if $locations_count > 1}
	<div style="margin:5px;">	
		<div>
			<h3 class="continent">{tr key="buss_title_country_busses"}  {$country.title}, {$country.continent_title}</h3>
			{foreach from=$locations item=new_location}
			{if $new_location.code != $location.code}
			<div class="acc">
				<div class="acctitle">
					<div style="float:left; height:16px; width:320px; overflow:hidden;"><a href="{url o='buss' m='location' region=$new_location.end_region_code location=$new_location.code}" title="{$new_location.title}">{$new_location.title}</a></div>
...
{if $country.description > ''}
	<div style="margin:5px;">	
		<div>
			<h3 class="continent">{tr key="buss_title_country_description"}  {$country.title}, {$country.continent_title}</h3>
			{$country.description}
		</div>
	</div>
{/if}
