...
<a href="{url o='charter' m='continent' continent=$region.continent_code}">{$region.continent_title}</a>  &raquo; 
<a href="{url o='charter' m='country' country=$region.country_code}">{$region.country_title}</a>  &raquo; 
<a href="{url o='charter' m='region' region=$region.code}">{$region.title}</a> 
...
<div style="margin:5px;">
	{foreach from=$locations item=location}
		<div class="acc">
			<div class="acctitle">
				<div style="float:left; height:16px; width:240px; overflow:hidden;"><a href="{url o='charter' m='location' location=$location.code}" title="{$location.title}">{$location.title}</a></div>
				<div style="float:right;width:80px;"><img src="{$img_path}star_{$location.accommodation_type_stars}.gif"/></div>					
				<div class="clearer"><span></span></div>
			</div>
			<div class="accbody">
				<div style="float:left;width:110px;">
					<div class="accthumb"><a href="{url o='charter' m='location' location=$location.code}" title="{$location.title}">{$location.location_thumb}</a></div>
				</div>
				<div style="text-align:right; padding-right:5px;"><span class="price">{tr key="charter_price"} {$location.price} {$location.currency_title}</span></div>													
...
{if $region.description > ''}
	<div style="margin:5px;">	
		<div>
			<h3 class="acc_title">{tr key="charter_title_region_description"}  {$region.title}, {$region.country_title}, {$region.continent_title}</h3>
			{$region.description}
		</div>
	</div>
{/if}