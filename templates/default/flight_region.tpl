...
<a href="{url o='flight' m='continent' continent=$region.continent_code}">{$region.continent_title}</a>  &raquo; 
<a href="{url o='flight' m='country' country=$region.country_code}">{$region.country_title}</a>  &raquo; 
<a href="{url o='flight' m='region' region=$region.code}">{$region.title}</a> 

<h3 class="acc_title">{tr key="flight_title_country_flights"}  {$region.title}, {$region.country_title}, {$region.continent_title} </h3>
<div style="margin:5px;">
	{foreach from=$locations item=location}
		<div class="acc">
			<div class="acctitle">
				<div style="float:left; height:16px; width:360px;"><a href="{url o='flight' m='location' location=$location.code}" title="{$location.title}">{$location.title|truncate:100}</a></div>
				<div class="clearer"><span></span></div>
			</div>
			<div class="accbody">
				<div style="text-align:right; padding-right:5px;"><span class="price">{tr key="flight_price"} {$location.price1} {$location.currency_title}</span></div>													
...
{if $region.description > ''}
	<div style="margin:5px;">	
		<div>
			<h3 class="acc_title">{tr key="flight_title_region_description"}  {$region.title}, {$region.country_title}, {$region.continent_title}</h3>
			{$region.description}
		</div>
	</div>
{/if}